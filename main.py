...
async def ask(question: str = Form(...), authorization: str | None = Header(None, alias="Authorization"), patient_id: str | None = Header(None, alias="Patient-Id")):
    ...
    try:
        ...
        return ApiResponse(ok=False, error={"code": "AUTH_EXPIRED", "message": "Phiên đăng nhập hết hạn."}).model_dump()
    except FileNotFoundError:
        return ApiResponse(ok=False, error={"code": "PATIENT_NOT_FOUND", "message": "Không tìm thấy hồ sơ."}).model_dump()
    except Exception as e:
        return ApiResponse(ok=False, error={"code": "UPSTREAM_UNAVAILABLE", "message": str(e)}).model_dump()
    ...
    return ApiResponse(ok=True, data=answer).model_dump()
